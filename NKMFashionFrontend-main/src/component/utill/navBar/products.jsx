import React, { useState, useEffect } from "react";
import axios from "axios";
import LinearProgress from "@mui/material/LinearProgress";
import Box from "@mui/material/Box";
import productIcon from "../../../img/product icon.jpg"; 

export default function Products() {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/api/findAllProduct`
        );
        if (
          response.data &&
          response.data.products &&
          Array.isArray(response.data.products)
        ) {
          setProducts(response.data.products);
        } else {
          setProducts([]);
        }
      } catch (error) {
        setProducts([]);
      } finally {
        setLoading(false);
      }
    };
    fetchProducts();
  }, []);

  // Helper to get price for any product type
  const getProductPrice = (product) => {
    if (product.ptype === "Single") {
      return product.productPrice;
    }
    if (product.ptype === "Variation") {
      // Try top-level productPrice first
      if (product.productPrice !== undefined && product.productPrice !== null) {
        return product.productPrice;
      }
      // Fallback: Try to extract from warehouse/variationValues
      if (product.warehouse && typeof product.warehouse === "object") {
        const warehouses = Object.values(product.warehouse);
        for (const warehouse of warehouses) {
          // Try new structure
          if (warehouse.variationValues && typeof warehouse.variationValues === "object") {
            const variations = Object.values(warehouse.variationValues);
            for (const variation of variations) {
              if (variation && variation.productPrice !== undefined) {
                return variation.productPrice;
              }
            }
          }
          // Try old structure (variation directly under warehouse)
          for (const key in warehouse) {
            if (
              warehouse[key] &&
              warehouse[key].productPrice !== undefined
            ) {
              return warehouse[key].productPrice;
            }
          }
        }
      }
    }
    return "N/A";
  };

  return (
    <div className="flex">
      {loading && (
        <Box
          sx={{
            width: "100%",
            position: "fixed",
            top: 0,
            left: 0,
            zIndex: 1000,
          }}
        >
          <LinearProgress />
        </Box>
      )}
      <div className="ml-64 flex-1">
        <nav className="bg-white shadow">
          <div className="container mx-auto flex justify-between items-center py-4 px-6">
            <h1 className="text-xl font-bold">App Name</h1>
            <span className="text-gray-700 hover:text-blue-500">Products</span>
          </div>
        </nav>
        {!loading && (
          <div className="mt-6 mx-auto w-11/12 bg-[#eff3f7] p-6 rounded-lg shadow-lg">
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800">
              Products
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6">
              {products.map((product) => (
                <div
                  key={product._id}
                  className="flex flex-col items-center p-4 bg-white shadow rounded-lg"
                >
                  <img
                    src={product.image || productIcon}
                    alt={product.name}
                    className="w-24 h-24 rounded-full mb-3 border-2 border-gray-300 object-cover"
                  />
                  <h3 className="text-lg font-semibold text-center text-gray-700">
                    {product.name}
                  </h3>
                  <p className="text-sm text-gray-500">
                    Rs. {getProductPrice(product)}
                  </p>
                </div>
              ))}
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
