            ...
            <div className="bg-white mt-[20px] pb-2 w-full rounded-2xl px-8 shadow-md">
                <div className="flex  flex-1 flex-col px-2 py-12 lg:px-8">
                    <form >
                        <div className="flex w-full space-x-5">
                            ...
                            <div className="flex-1">
                                <label className="block text-sm font-medium leading-6 text-gray-900 text-left">From <span className='text-red-500'>*</span></label>
                                <select
                                    id="warehouse"
                                    name="warehouse"
                                    value={warehouseFrom}
                            ...
                            <div className="flex-1"> {/* Use flex-1 to allow the field to take full width */}
                                <label className="block text-sm font-medium leading-6 text-gray-900 text-left">To <span className='text-red-500'>*</span></label>
                                <select
                                    id="warehouse"
                                    name="warehouse"
                                    value={warehouseTo}
                                ...
                                >
                                    <option value="">Select a warehouse</option>
                                    {Array.isArray(warehouseData) && warehouseData.map((wh) => (
                                        <option key={wh.name} value={wh.name}>
                                            {wh.name}
                                        </option>
                                    ))}
                    ...
                    <div className="flex-1 mt-5">
                        {/* Input Field */}
                        <label className="block text-sm font-medium leading-6 text-gray-900 text-left">Product : <span className='text-red-500'>*</span></label>
                        <input
                            id="text"
                            name="text"
                            type="text"
                            ...
                            <tbody>
                                {selectedProduct.map((product, index) => (
                                    <tr key={index}>
                                        ...
                                        {/* Delete Action */}
                                        <td className="px-6 py-4 text-left whitespace-nowrap text-right text-sm text-gray-500">
                                            <button
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-2">
                        <div>
                            <label className="block text-sm text-left font-medium text-gray-700">Status: <span className='text-red-500'>*</span></label>
                            <select
                                value={orderStatus}
                                onChange={(e) => setOrderStatus(e.target.value)}
                                className="searchBox w-[100%] pl-10 pr-2 py-2 border border-gray-300 rounded-md shadow-sm focus:border-transparent"
                            >
                    ...
                    <button onClick={() => handleCreateTransfer(
                        calculateTotal().toFixed(2), warehouseFrom, warehouseTo, selectedProduct, date, discountType, discount, shipping, tax, orderStatus, setResponseMessage, setError, navigate)} className="mt-10 submit  w-[200px] text-white rounded py-2 px-4">
                        Save Transfer
                    </button>
                </div>
