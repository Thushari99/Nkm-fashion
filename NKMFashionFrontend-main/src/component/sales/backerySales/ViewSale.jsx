...
function ViewBackerySale() {
    ...
    const SingleSale = combinedProductData.filter(sale => sale.saleType === 'POS');
    const EndOfTheDaySale = combinedProductData.filter(sale => sale.saleType === 'Non-POS');
                                ...
                                {data.map((sale) => (
                                    <tr key={sale._id}>
                                        ...
                                        <td className="px-6 py-4 text-left whitespace-nowrap text-m text-gray-900">
                                            <div className='flex items-center'>
                                                {permissionData.edit_sale && (
                                                    <div>
                                                        {activeTab === 'POS' && (
                                                            <Link to={`/editBackerySale${sale._id}`}
                                                ...
                                                {openPopupId === sale._id && (
                                                    <div ref={popupRef} className="absolute right-0 mt-2 w-48 bg-white border rounded shadow-lg z-10">
                                                        <ul ref={targetRef} className="text-sm text-gray-700">
                                                            ...
                                                            {activeTab === 'POS' && permissionData.show_payment && (
                                                                <li
                                                                    key={sale._id}
                                                            ...
                                                            {activeTab === 'POS' && permissionData.return_sale && (
                                        ...
                                        {openViewSale === sale._id && (
                                            ...
                                            >
                                                <div
                                                    className=" overflow-y-auto scroll-container bg-white w-[1300px] max-h-[90vh] overflow-auto p-8 pt-8 rounded-md shadow-lg mt-40 mb-10 z-100"
                                                    data-aos="fade-down"
                                                >
                                                    <div
                                                        ref={targetRef}
                                                        className="w-[1250px] max-h-[80vh] p-10 bg-white"
                                                        style={{
                                                            margin: '0 auto',
                                                            padding: '15px',
        ...
        <div className='relative background-white absolute top-[80px] left-[18%] w-[82%] h-[100vh] p-5'>
            <div className='flex justify-between mb-4 '>
                ...
                {permissionData.create_sale && (
                    <div>
                        <Link
                            to={'/createEndOfTheDaySale'}
                            className="submit flex-none rounded-md px-4 py-2.5 text-sm font-semibold text-white shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 w-60 text-center relative z-10"
                        >
                            Create Sale
                        </Link>
                    </div>
            ...
            <div>
                <div className="flex mb-0">
                    <button
                        className={`px-4 py-2 rounded-tl-md ${activeTab === 'Non-POS' ? 'bg-gray-500 text-white' : 'bg-gray-200 text-gray-700'}`}
                        onClick={() => setActiveTab('Non-POS')}
                    >
                        End of the day sales
                    </button>
                    <button
                        className={`px-4 py-2 rounded-tr-md  ${activeTab === 'POS' ? 'bg-gray-500 text-white' : 'bg-gray-200 text-gray-700'}`}
                ...
                {loading ? (
                    ...
                    <>
                        {activeTab === 'POS' && renderTable(SingleSale, 'Single Sales')}
                        {activeTab === 'Non-POS' && renderTable(EndOfTheDaySale, 'End of the day sales')}
                    </>
                )}
            </div>
...
export default ViewBackerySale;