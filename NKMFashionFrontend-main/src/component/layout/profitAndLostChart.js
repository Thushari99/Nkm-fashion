...
const CombinedSalesChart = () => {
    ...
    useEffect(() => {
        if (selectedPeriod === 'monthly' && monthlySalesData.length && monthlyPurchaseData.length) {
            renderChart(monthlySalesData, monthlyPurchaseData, `Monthly Sales and Purchases (${currency})`, formatMonthlyLabels);
        } else if (selectedPeriod === 'weekly' && weeklySalesData.length && weeklyPurchaseData.length) {
            renderChart(weeklySalesData, weeklyPurchaseData, `Weekly Sales and Purchases (${currency})`, formatWeeklyLabels);
        } else if (selectedPeriod === 'daily' && dailySalesData.length && dailyPurchaseData.length) {
            renderDailyChart(dailySalesData, dailyPurchaseData, `Daily Sales and Purchases (${currency})`);
        }
    ...
    const renderChart = (salesData, purchaseData, label, formatLabels) => {
        ...
        const sales = filledSalesData.map(item => item.pureProfit);
    ...
    const renderDailyChart = (salesData, purchaseData, label) => {
        ...
        const salesMap = new Map(salesData.map(item => [item.date, item.pureProfit]));
