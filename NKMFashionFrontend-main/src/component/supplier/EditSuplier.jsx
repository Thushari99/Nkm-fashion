...
function EditSuplierBody() {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'username') {
            if (!value.includes('@')) {
                setErrors(prevErrors => ({
                    ...prevErrors,
                    username: 'Username must be a valid email address containing "@"'
                }));
            } else {
                setErrors(prevErrors => ({
                    ...prevErrors,
    ...
    const handleSubmit = (e) => {
        ...
        // (e.g., mobile, username validations from handleChange)
        if (Object.values(errors).some(err => err !== '')) {
            // Optionally, you can display a toast error here
        ...
        axios.put(`${process.env.REACT_APP_BASE_URL}/api/editSuplierProfileByAdmin`, formDataToSubmit)
            ...
            .catch(error => {
                console.error('Error updating user. Try again later:', error);
