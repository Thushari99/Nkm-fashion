...
function ViewPurchaseBody() {
    ...
    const showConfirmationModal = (purchaseId) => {
        setPurchaseToDelete(purchaseId); // Set the sale ID to be deleted
    ...
    const searchPurchase = async (query) => {
        ...
        try {
            if (!query.trim()) {
                ...
                setSearchedSuplierPurchased(purchasedData); // Reset to the initial list
                setSuccessStatus("");
                return;
            }
    ...
    const handleInputChange = (e) => {
        ...
        debounceTimeout.current = setTimeout(() => {
            if (value.trim() === "") {
                setError("");
                setSuccessStatus("");
                setSearchedSuplierPurchased(purchasedData); // Reset to full list
            } else {
                searchPurchase(value); // Call the search API with the entered query
            }
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {combinedProductData.map((purchased) => (
                                <tr key={purchased._id}>
                                    ...
                                    <td className="px-6 py-5 whitespace-nowrap text-m text-left text-gray-900"><p className='rounded-[5px] text-center p-[6px] bg-red-100 text-red-500'>{purchased.invoiceNumber}</p></td>
                                    <td className="px-6 py-5 whitespace-nowrap text-m text-left text-gray-900"><p className='rounded-[5px] text-center p-[6px] bg-red-100 text-red-500'>{purchased.supplier}</p></td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-left text-m text-gray-900">{currency} {' '} {formatWithCustomCommas(purchased.grandTotal)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-left text-m text-gray-900">{currency} {' '} {formatWithCustomCommas(purchased.paidAmount)}</td>
                                            ...
                                            {openPopupId === purchased._id && (
                                                <div ref={popupRef} className="absolute right-0 mt-2 w-48 bg-white border rounded shadow-lg z-10">
                                                    <ul className="text-sm text-gray-700">
                                                        ...
                                                        {permissionData.return_purchase && (
                                        ...
                                        <div ref={popupRef} className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                                                ...
                                                <div ref={targetRef} className="w-[1250px] p-10 bg-white" style={{ margin: '0 auto', padding: '15px', boxSizing: 'border-box' }}>
                                                    ...
                                                    <div className="grid grid-cols-3 gap-8 text-gray-700">
                                                        ...
                                                        <div className="border-r pr-8">
                                                            ...
                                                            <p className="mb-1 text-left"><i className="fas fa-user ml-2 mr-2 text-gray-400"></i><span className="font-medium">Suplier:</span> {purchased.supplier}</p>
                                            ...
                                            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                                                <div className="bg-white w-[800px] h-[620px] overflow-auto p-8  pt-4 rounded-md shadow-lg mt-28 mb-10" data-aos="fade-down">
                                                                ...
                                                                {paymentData && paymentData.length > 0 ? (
                                                                    paymentData.map((pd) => (
                                                                        <tr key={pd._id}>
                                                                            ...
                                                                            <td className="px-4 py-4 whitespace-nowrap text-left text-m text-gray-900">{currency}{' '} {purchased.grandTotal}</td>
                                                    ...
                                                    <div className='flex items-center'>
                                                        <button onClick={() => handleEditClick(purchased._id)} className="px-6 flex items-center submit mt-5 text-white mr-2 h-[40px] rounded-md shadow-md transition">Create Payment</button>
